...
# export TMP in order to use it in subprocess
	...
	for namespace in secrets/core/*; do
		...
		if [[ -n "$$rows" ]]; then
			...
			sed -i '$$d' $$tempfile
			sed -i '/^$$/d' $$tempfile
			cat "$$tempfile" > kustomize/core/namespaces/$${namespace}/overlays/$${cluster}-$${env}/secrets.yaml
			cd kustomize/core/namespaces/$${namespace}/overlays/$${cluster}-$${env}
	...
	for namespace in secrets/addons/*; do
		...
		if [[ -n "$$rows" ]]; then
			...
			sed -i '$$d' $$tempfile
			sed -i '/^$$/d' $$tempfile
			cat "$$tempfile" > kustomize/addons/namespaces/$${namespace}/overlays/$${cluster}-$${env}/secrets.yaml
			cd kustomize/addons/namespaces/$${namespace}/overlays/$${cluster}-$${env}
			kustomize edit add resource secrets.yaml
			cd $$OLDPWD
		fi
	done
